...
void FormatFunc(char *equation, bool *status) {
  ...
  while (equation[pos] != '\0') {
    ...
    if (isalpha(equation[pos]) && equation[pos] != ' ' &&
        equation[pos] != '.') {
      ...
      if (strlen(equation_in) > 0)
        FormatFunc(equation_in, status);
      // calculation in F()
    ...
    if (isOper(equation[pos])) {
      if (operation == NULL) {
        ...
        if (GetPriority(operation->data) < GetPriority(equation[pos])) {
          ...
          if (GetPriority(operation->data) == 3 &&
              GetPriority(equation[pos]) == 3) {
            operation = s_push(operation, equation[pos]);
          } else {
            while ((operation != NULL) && (GetPriority(operation->data) >=
                                           GetPriority(equation[pos]))) {
...
double GetResult(char *equation, bool *status) {
  ...
  При нахождении бинарного оператора, берём два последних значения из стека в
  обратном порядке; При нахождении унарного оператора, в данном случае -
  унарного минуса, берём последнее значение из стека и вычитаем его из нуля, так
  как унарный минус является правосторонним оператором; Последнее значение,
  после отработки алгоритма, является решением выражения.
  */
  ...
  while (equation[pos] != '\0') {
    ...
    if (isOper(equation[pos])) {
      if (equation[pos] == '~') {
        ...
        first_tmp_num = cs_pop(&buffer);
        sprintf(tmp_char, "%lf",
                ExecutableInstructions(equation[pos], first_tmp_num,
                                       second_tmp_num, status));
        if (!status)
          break;
        buffer = cs_push(atof(tmp_char), buffer);
        memset(tmp_char, '\0', sizeof(tmp_char));
