...
public:
    DepositModel() noexcept {
        ...
        // supplement_date_arr_[3] = {0};
        supplement_type_ = ONETIME;
        supplement_date_ = 0;
        supplement_value_ = 0;

        // elimination_date_arr_[3] = {0};
    ...
    void UpdateModel() noexcept {
        ...
        for (int i = 0; i <= timeframe_; i++) {
            if (supplement_type_ == ONETIME && supplement_value_ > 0) { // разовое
                addSupp(balance);
                break;
            } else if (supplement_type_ == MONTH) { // month
            ...
            } else if (supplement_type_ == QUATER) { // quater
                addSupp(balance);
                ...
                supplement_date_ = 0;
                parserDate(true, false);
            } else if (supplement_type_ == YEAR) { // year
                addSupp(balance);
                supplement_date_arr_[2]++;
                supplement_date_ = 0;
                parserDate(true, false);
            }
            if (elimination_type_ == ONETIME && elimination_value_ > 0) { // разовое
                addElim(balance);
                break;
            } else if (elimination_type_ == MONTH) { // month
                addElim(balance);
            ...
            } else if (elimination_type_ == QUATER) { // quater
                addElim(balance);
                ...
                elimination_date_ = 0;
                parserDate(false, true);
            } else if (elimination_type_ == YEAR) { // year
                addElim(balance);
                elimination_date_arr_[2]++;
                elimination_date_ = 0;
                parserDate(false, true);
        ...
        if (!capitalisation_) { // добавить пополнения и снятия и итоговая сумма
            // с их учетом
            ...
            if (timeframe_type_ == TDAY) { // days
                for (int i = 1; i <= timeframe_; i++) {
                    balance[i] += balance[i - 1] +
                                  balance[i - 1] * (((interest_rate_ / 100.0) / 365));
                }
            } else if (timeframe_type_ == TMONTH) { // moth
                for (int i = 1; i <= timeframe_; i++) {
                    balance[i] += balance[i - 1] +
                                  balance[i - 1] * (((interest_rate_ / 100.0) / 365) *
                                                    kDayInMonth[(i - 1) % 12]);
                }
            } else if (timeframe_type_ == TQUATER) { // quater
                for (int i = 1; i <= timeframe_; i++) {
                    balance[i] += balance[i - 1] +
                                  balance[i - 1] * (((interest_rate_ / 100.0) / 365) *
                                                    kDayInQuater[(i - 1) % 4]);
...
private:
        ...
        if (timeframe_type_ == TDAY) {
            ...
            if (supplement_value_ != 0.0 && supp) {
                supplement_date_ = supplement_date_arr_[1] +
                                        12 * (supplement_date_arr_[2] - 2024);
            }
            if (elimination_value_ != 0.0 && elim) {
                elimination_date_ = elimination_date_arr_[1] +
                                         12 * (elimination_date_arr_[2] - 2024);
            }
    ...
    void addSupp(double *balance) noexcept {
        for (int i = 0; i <= timeframe_; i++) {
            if (supplement_date_ - 1 == i) {
                ...
                if (capitalisation_)
                    total_profit_ -= supplement_value_;
            }
        }
    };
    ...
    void addElim(double *balance) noexcept {
        for (int i = 0; i <= timeframe_; i++) {
            if (elimination_date_ - 1 == i) {
                ...
                if (capitalisation_)
                    total_profit_ += elimination_value_;
            }
        }
    };
    ...
    void Tax() noexcept {
        if (total_profit_ > 1000000 * 0.075) {
            total_tax_ =
                (total_profit_ - 1000000 * 0.075) * tax_rate_ / 100;
