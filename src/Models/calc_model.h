 ...
 private:
  ...
  void ReplaceFunctions() {
    ...
    while (eq_pos < equation_.length()) {
      ...
      equation_ = equation_.substr(0, f_begin) + _tmp +
                  equation_.substr(f_end, equation_.length());
      delete f_model;
      delete f_name;
      delete f_equation_in;
      --eq_pos;
  ...
  void ReplaceX(calc_value x) noexcept {
    ...
    while (pos < equation_.length()) {
      if (equation_.at(pos) == 'x') {
        equation_ = equation_.substr(0, pos) + _tmp +
                    equation_.substr(pos + 1, equation_.length());
      }
      pos++;
    }
  };
  ...
  void PreparatePN() {
    ...
    while (eq_pos < equation_.length()) {
      if (isOper(equation_.at(eq_pos))) {
        if (opers_.empty() ||
            (PriorityPN(opers_.top()) < PriorityPN(equation_.at(eq_pos))) ||
            (PriorityPN(opers_.top()) == 3 &&
             PriorityPN(equation_.at(eq_pos)) == 3)) {
    ...
    while (eq_pos < equation_.length()) {
      if (isOper(equation_.at(eq_pos))) {
        if (equation_.at(eq_pos) == '~') {
          ...
          nums_.pop();
          nums_.push(ExecutableInstructions(equation_.at(eq_pos), first_tmp_num,
                                            second_tmp_num));
          if (!status_) {
            break;
          }
   ...
   */
  calc_value ExecutableInstructions(char op, calc_value first,
                                    calc_value second) {
