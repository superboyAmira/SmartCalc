...
public:
    ...
    void UpdateModel() noexcept {
        if (type_ == ANN) {
            ...
            overpayment_ =
                (month_pay_ * months_) - amount_;
            total_ = overpayment_ + amount_;
        } else {
            double pay_main_credit = (amount_ / months_);
            month_pay_ =
                pay_main_credit +
                ((amount_ * (rate_ / 100) * 30.416666667) / 365);
            month_pay_ =
                (int)(month_pay_ * 100 + 0.5) / 100.0;
            total_ += month_pay_;
            ...
            for (double max_sum = amount_ - pay_main_credit; pos > 0;
                ...
                month_pay_diff_min_ =
                    pay_main_credit +
                    ((max_sum * (rate_ / 100) * 30.416666667) / 365);
                month_pay_diff_min_ =
                    (int)(month_pay_diff_min_ * 100 + 0.5) / 100.0;
