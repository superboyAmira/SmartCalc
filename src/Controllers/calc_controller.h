...
class CalcController {
 public:
  ...
  void DefaultView(QLabel* label) const noexcept { label->setText(""); }

  void upadteViewModel(QLabel* label, const QString equation, const double x,
                       QCheckBox* x_mode) {
  ...
  void updateGraph(QCustomPlot* graphic, QLabel* label, const double x_max,
                   const double x_min, const double y_max, const double y_min) {
    graphic->clearPlottables();
    if (label->text().length() == 0 ||
        (x_max == 0 && x_min == 0 && y_min == 0 && y_max == 0)) {
