...
public:
    void updateViewModel(QLineEdit* amount, QLineEdit* rate, QComboBox* timeframe, QComboBox* type, QLabel* month_pay, QLabel* overpayment, QLabel* total) {
        bool status = true;
        model.clear();
        ...
        if (model.GetType() == credit_t::DIFF) {
            month_pay->setText(QString::asprintf("%.2lf", model.GetMonthPay()) + "..." + QString::asprintf("%.2lf", model.GetMinDiffMonthPay()));
        } else {
